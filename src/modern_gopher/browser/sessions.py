...
class SessionManager:
    ...
    def _load_sessions(self) -> Dict[str, BrowserSession]:
        ...
        try:
            ...
            for session_id, session_data in sessions_data.items():
                try:
                    sessions[session_id] = BrowserSession.from_dict(
                        session_data)
    ...
    def _save_sessions(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Saved {len(self.sessions)} sessions to {self.session_file}")
            return True
    ...
    def save_session(self, browser_state: Dict[str, Any],
        ...
        if self._save_sessions():
            logger.info(
                f"Session '{session_name}' saved with ID: {session_id}")
            return session_id
        else:
    ...
    def export_sessions(self, export_path: Union[str, Path]) -> bool:
        ...
        try:
            export_data = {
                ...
                'sessions': {
                    sid: session.to_dict() for sid,
                    session in self.sessions.items()}}

            with open(export_path, 'w', encoding='utf-8') as f:
                json.dump(export_data, f, indent=2, default=str)

            logger.info(
                f"Exported {len(self.sessions)} sessions to {export_path}")
            return True
    ...
    def import_sessions(
            self, import_path: Union[str, Path], merge: bool = True) -> bool:
        """
        ...
        try:
            ...
            for session_id, session_data in imported_sessions.items():
                try:
                    ...
                    logger.warning(
                        f"Failed to import session {session_id}: {e}")

            if self._save_sessions():
                logger.info(
                    f"Imported {imported_count} sessions from {import_path}")
                return True
            else:
