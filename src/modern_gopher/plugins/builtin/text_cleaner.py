...
class TextCleaner(ContentProcessor):
    ...
    def metadata(self) -> PluginMetadata:
        return PluginMetadata(
            ...
            description=(
                "Cleans up text content by removing excessive " "whitespace and fixing formatting"
            ),
        )
    ...
    def can_process(self, content, metadata: Dict[str, Any]) -> bool:
        ...
        # Check content type if specified - allow text or if not specified
        content_type = metadata.get("content_type")
        if content_type and content_type not in ("text", "markdown"):
            return False
    ...
    def process(self, content: str, metadata: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        ...
        if self.config.get("remove_trailing_whitespace", True):
        ...
        if self.config.get("limit_blank_lines", True):
            max_blank_lines = self.config.get("max_consecutive_blank_lines", 2)
        ...
        if self.config.get("strip_content", True):
